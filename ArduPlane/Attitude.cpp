...
Vector3<float> Plane::stabilize_get_rpy_out()
{
    ...
    //迎角太大直接禁止滚转，并且副翼回中
    if (aoa > 5.5) {
        nav_roll_cd = nav_roll_cd * 0.5;
    }     
    if (aoa > 7) {
        nav_roll_cd = nav_roll_cd * 0.1;
    }   
    ...
    //以下俯仰限制 起飞时候稍微推一点点的杆 不要出现抬头趋势，逐渐达到稳态再抬头(源码)
    ...
    */
    ...
    // if (demanded_pitch < -radians(20.0f)) {
    //     demanded_pitch = - radians(20.0f);
    // } else if (demanded_pitch > radians(20.0f)) {
    //     demanded_pitch = radians(20.0f);
    // } 
    //以上俯仰限制
    ...
    float out_y_test = _lastout.y + 0.5 * aoa;
    ...
    constrained_out.y = map_range(out_y_test, -25.0f, 25.0f, -4500, 4500);
