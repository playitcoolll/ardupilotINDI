...
// #include <A_kalman/my_kalman.h>     // Range finder library
...
private:
    ...
    */
    ...
    // 没有找到home的赋值信息，可能是飞控开机的位置就被参数里定义为home？
    ...
    void stabilize_roll_my();
    void stabilize_pitch_my();
    float stabilize_roll_get_roll_out_my();
    float stabilize_pitch_get_pitch_out_my();
    ...
    void stabilize_rpy();
    Vector3<float> stabilize_get_rpy_out();
    Vector3<float> stabilize_get_rpy_out(float pdot, float qdpt);
    Vector3<float> stabilize_get_rpy_out_2(float roll, float pitch);
    Vector3f _lastout = {0,0,0};
    Vector3f multiplyMatrixVector(const float matrix[3][3], const Vector3f& vec); 
    float _get_coordination_rate_offset_my(float &aspeed, bool &inverted) const;
    float map_range(float value, float from_min, float from_max, float to_min, float to_max);
    //源码PID需要：
    AC_PID rate_pid_testx{1.0f, 0, 3.0f, 0.345, 0.666, 3, 0, 12, 150, 1};
    AC_PID rate_pid_testy{1.0f, 0, 3.0f, 0.345, 0.666, 3, 0, 12, 150, 1};
    // AC_PID rate_pid_test{0.04, 0.15, 0, 0.345, 0.666, 3, 0, 12, 150, 1};
    //自己PID需要：
    float derivative;
    float error_my = 0;
    float previous_error = 0;
    float derivative_roll;
    float error_my_roll = 0;
    ...
    float lastq = 0;
    float _derivativeq = 0;
    float _target = 0;
    float _error = 0;


    // float dt_my_test;
    // Vector3d measurement_my_test = {0,0,0}; 
    float lastderivativeq = 0; 
