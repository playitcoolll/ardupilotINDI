...
float AC_PID::update_all(float target, float measurement, float dt, bool limit, float boost)
{
    ...
    if (_flags._reset_filter) {
        ...
        // hal.console -> printf("111111111111111111111\n");

    } else {
        // hal.console -> printf("222222222222222222222\n");
...
float AC_PID::update_all_my_y(float target, float measurement, float dt, float aspeed)
{
    // don't process inf or NaN
    if (!isfinite(target) || !isfinite(measurement)) {
        return 0.0f;
    ...
    if(aspeed > 0.0f){
        boost_pitch_d  = 0.3f - 0.3f/20.0f*((abs(aspeed)));
    }
    boost_pitch_d = constrain_float(boost_pitch_d, 0.005f,0.3f);

    float P_out = (_error * boost_pitch_p/2.0f);
    float D_out = (_derivative * boost_pitch_d/2.0f);
    // calculate slew limit modifier for P+D
    _pid_info.Dmod = _slew_limiter.modifier((_pid_info.P + _pid_info.D) * _slew_limit_scale, dt);
    // hal.console -> printf("%d\n",_slew_limit_scale);//这个_slew_limit_scale是1，但是用官方sitl模型是45
    _pid_info.slew_rate = _slew_limiter.get_slew_rate();
...
float AC_PID::update_all_my_x(float target, float measurement, float dt, float aspeed)
{
    // don't process inf or NaN
    if (!isfinite(target) || !isfinite(measurement)) {
        return 0.0f;
    ...
    float boost_roll_p = 1.0f/ 2.0f;
    if(aspeed > 0.0f){
        boost_roll_p =1.0f/ 2.0f - 1.0f/ 2.0f/20.0f*((abs(aspeed)));
    }
    boost_roll_p = constrain_float(boost_roll_p, 0.005f,1.0f);

    float boost_roll_d = 0.3f/ 2.0f;
    if(aspeed > 0.0f){
        boost_roll_d =0.3f/ 2.0f - 0.3f/ 2.0f/20*((abs(aspeed)));
    }
    boost_roll_d = constrain_float(boost_roll_d, 0.05f,0.3f);

    float P_out = (_error * boost_roll_p );
    float D_out = (_derivative * boost_roll_d);
    // calculate slew limit modifier for P+D
    _pid_info.Dmod = _slew_limiter.modifier((_pid_info.P + _pid_info.D) * _slew_limit_scale, dt);
    _pid_info.slew_rate = _slew_limiter.get_slew_rate();
